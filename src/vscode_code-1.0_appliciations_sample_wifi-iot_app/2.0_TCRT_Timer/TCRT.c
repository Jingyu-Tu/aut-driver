...
// (3) 定义相关变量
#define GPIOI 13   // 左传感器 GPIO13
#define GPIOR 14   // 右传感器 GPIO14

uint32_t exec1;        // 时间变量
osTimerId_t id1;       // 创建时间ID
uint32_t timerDelay1;  // 创建延时时间
...
{
    (void)arg;
    // 循环执行获取左右两个传感器的任务，并且每次获取之间会等待2秒钟
    get_tcrt5000_value();
...
void get_tcrt5000_value(void)
{
    ...
    GpioGetInputVal(GPIOI, &id_status); // 获取GPIO13引脚的输入电平值
...
static void TCRTTask(void)
{
    ...
    if (id1 != NULL)
    {
        ...
        // 设置定时器超时时间5秒（50U=500ms，100U=1S，这里5U=50ms? 实际根据注释5U=50ms）
        timerDelay1 = 5U;
        // 定时器开启并获取返回参数
        status = osTimerStart(id1, timerDelay1);
        if (status != osOK)
        {
