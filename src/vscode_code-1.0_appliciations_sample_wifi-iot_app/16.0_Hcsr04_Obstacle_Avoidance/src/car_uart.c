...
bool CarUart_SendByte(uint8_t byte)
{
    // 使用现有的 UartWrite 函数发送一个字节
    return UartWrite(CAR_UART_INDEX, &byte, 1) == 1;
}
...
uint8_t GetSequence(void) {
    g_seq++;
    if (g_seq > 0xFE) g_seq = 0;  // 防止溢出
    return g_seq;
}

bool CarUart_SendProtocolCommand(uint8_t cmd) {
    uint8_t frame[5];
    uint8_t seq = GetSequence();
    uint8_t xor_check = 0xA5 ^ 0x5A ^ seq ^ cmd;
