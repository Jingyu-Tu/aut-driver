...
static void ObstacleTask(void *argument)
{
    ...
    {
        OBSTACLE_THRESHOLD_CM, CLEAR_THRESHOLD_CM,STOP_DURATION_MS, TURN_INTENT_HOLD_MS
    };
    ...
    ObstacleController_Init(&controller, &config, nowMs,hi_get_us() ^ 0xA341316CU);
    ...
    while (1) {
        ...
        command = ObstacleController_Update(&controller, nowMs, decisionValid,distanceCm);

        if (!CarUart_SendProtocolCommand(command)) {
